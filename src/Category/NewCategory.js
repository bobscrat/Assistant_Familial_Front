                ...
                <Form.Group inline widths='equal'>                  
                    <Form.Group >                      
                      ...
                      <Form.Input required name="colorName" placeholder="couleur de la catégorie" value={colorName} onChange={this.handleChange}>
                       ...
                       <Label tag style={{"backgroundColor" : this.state.colorName, "color" : "white"}} > {this.state.name} </Label>        
                    </Form.Group>
                </Form.Group>
                        ...
                        <Grid columns={2} padded > 
                                    { // en 2 colonnes, espace préservé, Grid.Colomn dans la map 
                                        this.state.categories.map(
                                            ...
                                            <Grid.Column>
                                              <Form.Group inline>
                                                <Label as='a' style={{"backgroundColor" : category.color, "color" : "#ffffff"}} tag>{category.name}</Label>
                                                <Form.Input required name={[category.id + 'name']} value={category.name} onChange={(e) => this.handleChangeCate(e, category.id)}
                                             
                                                />
                                                <Label circular style={{"backgroundColor" : this.state.colorNameModif, "color" : "white"}}  onClick={ () => this.handleClickShowColorPickerExist(category.id) } /> 
                                              </Form.Group>
              <ToggleDisplay show={this.state.showExist} key={category.id} >
                  <CirclePicker colors= {['#7947BD',' #983A7A', '#AE5A7C', '#BF4258', '#BA4E1D', '#A6645B', '#9A6D00', '#705A00', '#00891D', 
                                      '#32797C', '#007DA6', '#0061C1', '#54584B', '#106326', '#064B2D', '#005D71', '#40497C', '#663865', 
                                      '#713066', '#700C26', '#942A46', '#9A3921', '#AF5800', '#814B00'] }  
